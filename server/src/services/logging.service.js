...
class LoggingService {
  ...
  prepareUpdateLogs(
    ...
    changedFields.forEach((field) => {
      oldValues[field.name] = field.old_value;
      newValues[field.name] = field.new_value;
    });

    // Create a single log entry for all changed fields
    // We leave field_name null to indicate this is an aggregated update
    // We store the JSON stringified object of changes in old_value and new_value
    return [
      this.prepareLogData({
        table_name,
        record_id,
  ...
  async logUpdate(
    ...
    const logEntries = this.prepareUpdateLogs(
      table_name,
      record_id,
      old_record,
      new_record,
  ...
  getChangedFields(old_record, new_record) {
    ...
    const allFields = new Set([
      ...Object.keys(old_record || {}),
      ...Object.keys(new_record || {}),
    ]);
