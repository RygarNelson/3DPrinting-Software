...
import {
  clearLoggingContext,
  setLoggingContext,
} from "../middleware/loggingContext.js";
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const vendita = await VenditaRepository.findOne(
      req.params.id,
      projection,
      include
    );
  ...
  asyncHandler(async (req, res) => {
    if (req != null && req.body != null) {
      ...
      let includeCliente = {
        association: "cliente",
        required: false,
        attributes: ["etichetta"],
        where: { deletedAt: null },
      };

      let includeDettagliModello = {
        association: "modello",
        attributes: ["nome", "descrizione"],
        required: false,
        where: { deletedAt: null },
      };
      let includeDettagliStampante = {
      ...
      let includeBasette = {
        ...
        where: { deletedAt: null },
        required: false,
        attributes: ["id", "dimensione", "quantita", "stato_stampa"],
      };
      ...
      if (
        ...
        if (req.body.stato_stampa != null) {
          ...
          venditeByDettaglioConStatoStampa.rows.forEach((v) =>
            venditeIds.add(v.id)
          );
        }
            ...
            [
              {
                ...
                where: {
                  deletedAt: null,
                  stato_stampa: req.body.basetta_stato_stampa,
                },
              },
            ]
          );
          venditeByBasettaConStatoStampa.rows.forEach((v) =>
            venditeIds.add(v.id)
          );
        }
        ...
        whereOptions = {
          [Op.or]: [
            { id: { [Op.in]: Array.from(venditeIds) } },
            { link_tracciamento: { [Op.like]: `%${req.body.search}%` } },
          ],
        };
      }
      ...
      // Data vendita
      if (
        req.body.data_vendita &&
        req.body.data_vendita.value &&
        req.body.data_vendita.operator
      ) {
        switch (req.body.data_vendita.operator) {
          case "dateIs": {
            whereOptions.data_vendita = {
              [Op.eq]: req.body.data_vendita.value,
            };
            break;
          }
          case "dateIsNot": {
            whereOptions.data_vendita = {
              [Op.ne]: req.body.data_vendita.value,
            };
            break;
          }
          case "dateBefore": {
            whereOptions.data_vendita = {
              [Op.lte]: req.body.data_vendita.value,
            };
            break;
          }
          case "dateAfter": {
            whereOptions.data_vendita = {
              [Op.gte]: req.body.data_vendita.value,
            };
            break;
          }
        }
      ...
      if (
        ...
        req.body.data_scadenza.value &&
        req.body.data_scadenza.operator
      ) {
        switch (req.body.data_scadenza.operator) {
          case "dateIs": {
            whereOptions.data_scadenza = {
              [Op.eq]: req.body.data_scadenza.value,
            };
            break;
          }
          case "dateIsNot": {
            whereOptions.data_scadenza = {
              [Op.ne]: req.body.data_scadenza.value,
            };
            break;
          }
          case "dateBefore": {
            whereOptions.data_scadenza = {
              [Op.lte]: req.body.data_scadenza.value,
            };
            break;
          }
          case "dateAfter": {
            whereOptions.data_scadenza = {
              [Op.gte]: req.body.data_scadenza.value,
            };
            break;
          }
        }
      ...
      if (
        req.body.data_scadenza_spedizione &&
        req.body.data_scadenza_spedizione.value &&
        req.body.data_scadenza_spedizione.operator
      ) {
        switch (req.body.data_scadenza_spedizione.operator) {
          case "dateIs": {
            whereOptions.data_scadenza_spedizione = {
              [Op.eq]: req.body.data_scadenza_spedizione.value,
            };
            break;
          }
          case "dateIsNot": {
            whereOptions.data_scadenza_spedizione = {
              [Op.ne]: req.body.data_scadenza_spedizione.value,
            };
            break;
          }
          case "dateBefore": {
            whereOptions.data_scadenza_spedizione = {
              [Op.lte]: req.body.data_scadenza_spedizione.value,
            };
            break;
          }
          case "dateAfter": {
            whereOptions.data_scadenza_spedizione = {
              [Op.gte]: req.body.data_scadenza_spedizione.value,
            };
            break;
          }
        }
      ...
      // Totale Vendita
      if (
        req.body.totale_vendita &&
        req.body.totale_vendita.value &&
        req.body.totale_vendita.operator
      ) {
        switch (req.body.totale_vendita.operator) {
          case "equals": {
            whereOptions.totale_vendita = {
              [Op.eq]: req.body.totale_vendita.value,
            };
            break;
          }
          case "notEquals": {
            whereOptions.totale_vendita = {
              [Op.ne]: req.body.totale_vendita.value,
            };
            break;
          }
          case "lt": {
            whereOptions.totale_vendita = {
              [Op.lt]: req.body.totale_vendita.value,
            };
            break;
          }
          case "lte": {
            whereOptions.totale_vendita = {
              [Op.lte]: req.body.totale_vendita.value,
            };
            break;
          }
          case "gt": {
            whereOptions.totale_vendita = {
              [Op.gt]: req.body.totale_vendita.value,
            };
            break;
          }
          case "gte": {
            whereOptions.totale_vendita = {
              [Op.gte]: req.body.totale_vendita.value,
            };
            break;
          }
        }
      ...
      const vendite = await VenditaRepository.find(
        whereOptions,
        limit,
        offset,
        order,
      ...
      const andamentoUltimiTreMesi =
        await VenditaRepository.ottieniAndamentoUltimiTreMesi();
      const andamentoUltimiTreMesiSospese =
        await VenditaRepository.ottieniAndamentoUltimiTreMesiSospese();
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    if (req.body.id > 0 && req.body.stato_avanzamento != null) {
      const data = await VenditaRepository.modificaStatoVendita(
        req.body.id,
        req.body.stato_avanzamento
      );
      return res.status(200).json({
        success: true,
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    if (req.body.id > 0 && req.body.stato_avanzamento != null) {
      const data = await VenditaRepository.modificaStatoDettaglio(
        req.body.id,
        req.body.stato_avanzamento
      );
      return res.status(200).json({
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    if (req.body.id > 0 && req.body.stato_avanzamento != null) {
      const data = await VenditaRepository.modificaStatoBasetta(
        req.body.id,
        req.body.stato_avanzamento
      );
      return res.status(200).json({
...
router.get(
  "/andamento/:anno",
  asyncHandler(async (req, res) => {
    const data = await VenditaRepository.ottieniAndamentoVendite(
      req.params.anno
    );
    const totaleVendite = data.data.datasets[0].data.reduce(
      (acc, curr) => acc + curr,
      0
    );
    const totaleSpese = data.data.datasets[1].data.reduce(
      (acc, curr) => acc + curr,
      0
    );
    const totaleSospese = data.data.datasets[2].data.reduce(
      (acc, curr) => acc + curr,
      0
    );

    // Per ogni elemento del dataset delle vendite in sospeso, sommare il totale delle vendite
    data.data.datasets[2].data = data.data.datasets[2].data.map(
      (item, index) => {
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    for (const contoBancario of contiBancari) {
      const totaleVendite =
        await VenditaRepository.sommaVenditePerContoBancarioAnno(
          req.params.anno,
          contoBancario.id
        );
      result.push({
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    const riepilogo =
      await VenditaRepository.ottieniRiepilogoVenditeModelliPerMese(
        req.params.anno
      );
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    await VenditaRepository.modificaContoBancarioVendite(
      req.body.vendite_ids,
      req.body.conto_bancario_id
    );
    return res.status(200).json({
      success: true,
